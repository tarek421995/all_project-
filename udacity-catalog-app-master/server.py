...
def showCategory(category_name):
    ...
    '''
    categories = db.session.query(Category).order_by(db.asc(Category.id))
    category = db.session.query(Category).filter_by(name=urllib.parse.unquote(category_name)).one()
...
def editItem(item_name):
    ...
    POST: Update Item
    '''
    item = db.session.query(CategoryItem).filter_by(name=urllib.parse.unquote(item_name)).one()

...
url = input("Please enter the uri you want to access, \n If left blank the connection will be set to 'http://localhost:1234/rate-limited':   ")
if url == '':
	url = 'http://localhost:1234/rate-limited'
...
def SendRequests(url, req_per_minute):
	...
	while requests < req_per_minute:
		result = json.loads(h.request(url,'GET')[1].decode('utf-16'))
